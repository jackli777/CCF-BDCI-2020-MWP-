...
class Lang:
    ...
    def trim(self, min_count):  # trim words below a certain count threshold
        ...
        print('keep_words %s / %s = %.4f' % (
            len(keep_words), len(self.index2word), len(
                keep_words) / len(self.index2word)
        ))
...
# load the json data to list(dict())
def load_raw_data(filename, raw=True, operator_no_gt_thresh=-1, num_count_no_gt_4=False, use_eda=-1):
    processed_eda_dict = {}
    print("Reading lines...")
    ...
    if raw:
        ...
        counter_all = 0
        counter_all_div_by_0 = 0
        counter_special_case = 0
        # 如果限制equation中的运算符不超过thresh个，初始化统计指标
        if operator_no_gt_thresh > 0:
            counter_operator_gt_thresh = 0
        if num_count_no_gt_4:
            counter_len_num_list_gt_4 = 0
    for i, s in enumerate(tqdm(f)):
        js += s
        i += 1
        if i % every_line_num == 0:  # every 7 line is a json
            ...
            if not raw:
                ...
                # 一些特例：
                # train_corrected_processed.json
                # "id": "3943"，最大的六位数比最小的六位数大多少？
                # "id": "10578"，最大的一位数减最大的两位纯小数，差是多少？
                exclude_special_cases = [3943, 10578]
                # TODO 有些 "num_list": [] 也可以去掉；
                # 为了降低模型学习、预测的难度，还可以仅把那些question中的num_list中的num均在equation中的保留下来，
                # 或者想办法反过来过滤，将equation中的num必须都出现于question中的num_list中的，才保留下来，
                # 这样做的话，就是舍弃了所有包含尝试、隐含知识类的题目
                if (int(data_d["id"]) in exclude_special_cases) or (("doc_index_originality" in data_d.keys()) and (data_d["doc_index_originality"] != 'original') and (int(data_d["doc_index_originality"]) in exclude_special_cases)):
                    counter_special_case += 1
                    js = ''
                    continue

                # 使用num_count_no_gt_4这个设置，并发现num_list中的num大于4个
                if num_count_no_gt_4 and (len(data_d["num_list"]) > 4):
                    counter_len_num_list_gt_4 += 1
                    js = ''
                    continue

                # 首先假设num_list中没有分母是0的
                found_div_by_0 = False
                # 逐个排查，一旦发现，该条数据舍弃
                for item in data_d["num_list"]:
                    # /0.5这类的是合法的，仅/0是不合法的
                    if ('/0' in item) and ('/0.' not in item):
                        found_div_by_0 = True
                        break
                if found_div_by_0:
                    counter_all_div_by_0 += 1
                    js = ''
                    continue

                # 生成group_num的时候忘记把equation中的**替换回^了，这里补一下：
                data_d["equation"] = data_d["equation"].replace('**', '^')

                # 再额外处理下来自crawled的一些特例：3.14*(20/2)2-3.14*62
                # TODO 对“爬”到的表达式其实处理的是不够的
                # 源头：
                # data_process_scrapy_results_zybang.py
                # data_process_group_num.py
                data_d["equation"] = re.sub(
                    r'\((\d+/\d+)\)(\d+)', '((\\1)*\\2)', data_d["equation"])
                ...
                # print_cond = (i <= 10) and (len(fraction_num_set) > 0)

                # if print_cond:
                #     print(f'for i {i}, what the fraction_num_set is:')
                #     pprint.pprint(fraction_num_set)
                #     print()
                ...
                # if print_cond:
                #     print(
                #         f'for i {i}, what the fraction_num_list is:')
                #     pprint.pprint(fraction_num_list)
                #     print()

                # 如果限制equation中的运算符不超过thresh个
                if operator_no_gt_thresh > 0:
                    ...
                    # 如果该问题的运算符超过thresh个，直接舍弃该问题，不加入训练
                    if counter_operator > operator_no_gt_thresh:
                        counter_operator_gt_thresh += 1
                        js = ''
                        continue
                ...
                # if print_cond:
                #     print(
                #         f'for i {i}, what the fraction_num_list_ext is:')
                #     pprint.pprint(fraction_num_list_ext)
                #     print()

                # if print_cond:
                #     print(
                #         f'for i {i}, what the data_d["cleaned_text"] is before:')
                #     pprint.pprint(data_d["cleaned_text"])
                #     print('and what the data_d["equation"] is before')
                #     pprint.pprint(data_d["equation"])
                #     print()
                ...
                # if print_cond:
                #     print(
                #         f'for i {i}, what the data_d["cleaned_text"] is after:')
                #     pprint.pprint(data_d["cleaned_text"])
                #     print('and what the data_d["equation"] is after')
                #     pprint.pprint(data_d["equation"])
                #     print()
    ...
    print('counter_all_div_by_0: ', counter_all_div_by_0,
          'percentage: ', counter_all_div_by_0/counter_all)
    print('counter_special_case: ', counter_special_case,
          'percentage: ', counter_special_case/counter_all)
    if operator_no_gt_thresh > 0:
        print('counter_operator_gt_thresh: ', counter_operator_gt_thresh,
              'percentage: ', counter_operator_gt_thresh/counter_all)
    if num_count_no_gt_4:
        print('counter_len_num_list_gt_4: ', counter_len_num_list_gt_4,
              'percentage: ', counter_len_num_list_gt_4/counter_all)
    print()
...
def transfer_num(data, raw=True):  # transfer num into "NUM"
    ...
    for i, d in enumerate(tqdm(data)):
        ...
        if raw:
            ...
            seg = d["segmented_text_new"].strip().split(" ")
        equations = d["equation"][2:]

        # if i <= 10:
        #     print(f'for i {i}, what the seg is:')
        #     pprint.pprint(seg)
        #     print()
        ...
        for s in seg:
            ...
            # if (i <= 10) and (pos is not None):
            #     print(f'for i {i}, s {s} what the pos is:')
            #     pprint.pprint(pos)
            #     print('and the pos.start() is:')
            #     pprint.pprint(pos.start())
            #     print('and the pos.end() is:')
            #     pprint.pprint(pos.end())
            #     print()
        ...
        for num in nums:
            if raw:
                ...
                pattern_ = r"\(\d+\/\d+\)"
            if re.search(pattern_, num):
                nums_fraction.append(num)
        nums_fraction = sorted(
            nums_fraction, key=lambda x: len(x), reverse=True)
        ...
        pairs.append((input_seq, out_seq, nums, num_pos, d["group_num"]))
...
def prepare_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    print('second loop over pairs_trained:')
    for pair in tqdm(pairs_trained):
        num_stack = []
        for word in pair[1]:
            temp_num = []
...
def prepare_de_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    for pair in pairs_trained:
        ...
        train_pairs.append([input_cell, len(input_cell),
                            pair[1], 0, pair[2], pair[3], num_stack, pair[4]])
...
def get_single_batch_graph(input_batch, input_length, group, num_value, num_pos):
    ...
    for i in range(len(input_length)):
        ...
        graph_total = [graph_newc.tolist(), graph_greater.tolist(
        ), graph_lower.tolist(), graph_quanbet.tolist(), graph_attbet.tolist()]
...
def get_single_example_graph(input_batch, input_length, group, num_value, num_pos):
    ...
    graph_greater = get_greater_num_graph(
        max_len, sentence_length, num_list, id_num_list)
    graph_lower = get_greater_num_graph(
        max_len, sentence_length, num_list, id_num_list)
...
def prepare_train_batch(pairs_to_batch, batch_size):
    ...
    for batch in batches:
        ...
        graph_batches.append(get_single_batch_graph(
    ...
    while idx < len(ex):
        ...
        if (s == "*" or s == "+") and random.random() < rate:
            ...
            if lidx > 0 and ((s == "+" and ex[lidx - 1] == "-") or (s == "*" and ex[lidx - 1] == "/")):
                lidx -= 1
                ex = ex[:lidx] + ex[idx:ridx + 1] + \
                    ex[lidx:idx] + ex[ridx + 1:]
            else:
                ex = ex[:lidx] + ex[idx + 1:ridx + 1] + \
                    [s] + ex[lidx:idx] + ex[ridx + 1:]
            idx = ridx
        idx += 1
    return ex
        ...
        if (ex[idx] == "/" or ex[idx] == "*") and (ex[idx - 1] == "]" or ex[idx - 1] == ")"):
            ...
            if not flag_al:
                ...
                while temp_idx < idx - 1:
                    ...
                    if flag == 0:
                        if ex[temp_idx] == "+" or ex[temp_idx] == "-":
                            temp_res += ex[lidx: temp_idx] + \
                                [ex[idx]] + r_allo + [ex[temp_idx]]
                            lidx = temp_idx + 1
                    temp_idx += 1
        ...
        if ex[idx] == "*" and (ex[idx + 1] == "[" or ex[idx + 1] == "("):
            ...
            if not flag_al:
                ...
                while temp_idx < idx - 1:
                    ...
                    if flag == 1:
                        if ex[temp_idx] == "+" or ex[temp_idx] == "-":
                            all_res += l_allo + [ex[idx]] + \
                                ex[lidx: temp_idx] + [ex[temp_idx]]
                            lidx = temp_idx + 1
                    if flag == 0:
                        break
                    temp_idx += 1
